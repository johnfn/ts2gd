...
import ts, { updateTypeLiteralNode } from "typescript"
import fs from "fs"
...
function compile(sourceFile: ParsedSourceFile, project: TsGdProject): void {
  ...
  for (const { content, name } of result.enums ?? []) {
    fs.writeFileSync(
      sourceFile.gdPath.slice(0, -".gd".length) + "_" + name + ".gd",
      content
    )
  }
...
${project.assets
  .map(({ resPath, fsPath, className }) => `  '${resPath}': ${className}`)
  .join(",\n")}
}
...
async function buildNodePathsType(sceneFsPath: string, project: TsGdProject) {
  ...
  const classesInScene = scene.resources
    .filter((res) => res.resPath.endsWith(".gd"))
    .map((res) => {
      let result = project.sourceFiles.find(
        (source) => source.resPath === res.resPath
      )
  ...
  const allClassesInScene = [
    ...classesInScene,
    ...(project.mainScene.fsPath === sceneFsPath
      ? project.sourceFiles.filter((source) => source.isAutoload)
      : []),
  ]
...
`)
        ...
        assetFileContents += `  '${scenePath}': import("${associatedClass.tsFullPath.slice(
          0,
          -".ts".length
        )}").${script.type},\n`
      } else {
    ...
    assetFileContents += "}\n\n"
    assetFileContents += `import ${className} from './../${
      tsgdJson.source
    }/${path.basename(tsFullPath).slice(0, -".ts".length)}';
`
...
const getAutoloadFiles = () => {
  const projectFile = fs.readFileSync(
    path.join(tsgdPath, "project.godot"),
    "utf-8"
  )
...
const getProjectProperties = async (): Promise<TsGdProject> => {
  ...
  const sourceFilePaths = watchProgram
    .getProgram()
    .getSourceFiles()
    .map((source) => source.fileName)
    .filter((path) => !path.endsWith(".d.ts") && !path.includes("/test/"))
  const sourceFiles = sourceFilePaths.map((sourceFilePath) =>
    getSourceFile(sourceFilePath, autoloads)
  )
  ...
  let assets = allFiles
    ...
    .filter(
      (f) =>
        (f.endsWith(".tscn") || f.endsWith(".gd") || f.endsWith(".ttf")) &&
        !f.includes("/compiled/")
    )
    .map((fsPath) => {
  ...
  in ${fsPath}`)
        ...
        className = `import('${sourceFile.tsFullPath.slice(
          0,
          -".ts".length
        ...
        if (script) {
          let sourceFile = sourceFiles.find(
            (file) => file.resPath === script.resPath
          )

          className = `PackedScene<import('${sourceFile?.tsFullPath.slice(
            0,
            -".ts".length
          )}').${sourceFile?.className}>`
        } else {
