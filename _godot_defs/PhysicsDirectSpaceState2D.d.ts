 ...
 *
 * **Note:** Both the shape and the motion are supplied through a [PhysicsShapeQueryParameters2D] object. The method will return an array with two floats between 0 and 1, both representing a fraction of `motion`. The first is how far the shape can move without triggering a collision, and the second is the point at which a collision will occur. If no collision is detected, the returned array will be `[1, 1]`.
 *
*/
cast_motion(shape: PhysicsShapeQueryParameters2D): any[];

/** Checks the intersections of a shape, given through a [PhysicsShapeQueryParameters2D] object, against the space. The resulting array contains a list of points where the shape intersects another. Like with [method intersect_shape], the number of returned results can be limited to save processing time. */
collide_shape(shape: PhysicsShapeQueryParameters2D, max_results?: int): any[];
 ...
 *
 * `metadata`: The intersecting shape's metadata. This metadata is different from [method Object.get_meta], and is set with [method PhysicsServer2D.shape_set_data].
 *
 ...
 *
*/
get_rest_info(shape: PhysicsShapeQueryParameters2D): Dictionary;
 ...
 *
*/
intersect_shape(shape: PhysicsShapeQueryParameters2D, max_results?: int): any[];

  connect<T extends SignalsOf<PhysicsDirectSpaceState2D>, U extends Node>(signal: T, node: U, method: keyof U): number;
